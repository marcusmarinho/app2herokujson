...
export class OfferService {
    ...
    public getOfferByCategory(categoria: string): Observable<Offer[]> {
        let params;
        params = new HttpParams().set('ofertas', categoria);
        return this.http.get(`${URL_API}/ofertas` , {params: params})
            .pipe(
                map((res) => {
                    return res as Offer[];
                })
    ...
    public getHowUseOfferById(id: number): Observable<string> {
        let params;
        params = new HttpParams().set('id', String(id));
        return this.http.get(`${URL_API}/como-usar`, {params: params})
            .pipe(
                take(1),
                map((resp: Offer) => {
    ...
    public getWhereIsOfferById(id: number): Observable<string> {
        let params;
        params = new HttpParams().set('id', String(id));
        return this.http.get(`${URL_API}/onde-fica`, {params: params})
            .pipe(
                take(1),
                map((resp: Offer) => {
                    return resp[0].descricao as string;
